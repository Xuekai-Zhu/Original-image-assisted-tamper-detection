...
class ImageDataTrain(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        im_name2 = self.sal_list[item % self.sal_num]
        #print('---', im_name)
        gt_name = im_name.split('.')[0] + '_gt.png'   #self.sal_list[item % self.sal_num]#.split()[1]
        sal_image = load_image(os.path.join(self.sal_root, im_name))
        print(self.sal_root)
        sal_label = load_sal_label(os.path.join(self.sal_root + '_mask', gt_name))
        sal_image, sal_imgsal_label = cv_random_flip(sal_image, sal_label)
...
def get_loader(config, mode='train', pin=False):
    ...
    if mode == 'train':
        ...
        data_loader = data.DataLoader(dataset=dataset, batch_size=config.batch_size, shuffle=shuffle, num_workers=0, pin_memory=pin)
    else:
