...
class Solver(object):
    ...
    def build_model(self):
        ...
        if self.config.load == '':
            self.net.base.load_pretrained_model(torch.load('./SOD100k/' + self.config.pretrained_model))
        else:
    ...
    def test(self):
        ...
        for i, data_batch in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                multi_fuse = pred
                #print('-----------------', name)
                cv2.imwrite(os.path.join(self.config.test_fold, name + '.png'), multi_fuse)
        time_e = time.time()
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch):
            ...
            for i, data_batch in enumerate(self.train_loader):
                ...
                sal_loss_fuse = criterion(sal_pred, sal_label)
