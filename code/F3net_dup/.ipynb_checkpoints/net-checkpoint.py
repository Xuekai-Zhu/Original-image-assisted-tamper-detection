...
class F3Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.linearp12 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.linearp22 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)

        self.linearr22 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.linearr32 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.linearr42 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.linearr52 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)

        self.initialize()

    def forward(self, x,x2, shape=None):

        pred11, pred21,out21h, out31h, out41h, out51v=self.f(x)
        pred12, pred22,out22h, out32h, out42h, out52v=self.f(x2)

        out22h=torch.abs(out21h-out22h)
        out32h=torch.abs(out31h-out32h)
        out42h=torch.abs(out41h-out42h)
        out52v=torch.abs(out51v-out52v)
        pred22=torch.abs(pred21-pred22)
        pred12=torch.abs(pred11-pred12)


        pred1, pred2,out2h, out3h, out4h, out5v=self.f2(x)



        shape = x.size()[2:] if shape is None else shape

        pred12 = self.linearp12(pred12)
        pred22 = self.linearp22(pred22)
        out22h = self.linearr22(out22h)
        out32h = self.linearr32(out32h)
        out42h = self.linearr42(out42h)
        out52h = self.linearr52(out52v)

        pred1=pred1*(1-torch.sigmoid(pred12))
        pred2=pred2*(1-torch.sigmoid(pred22))
        out2h=out2h*(1-torch.sigmoid(out22h))
        out3h=out3h*(1-torch.sigmoid(out32h))
        out4h=out4h*(1-torch.sigmoid(out42h))
        out5v=out5v*(1-torch.sigmoid(out52h))

        pred12 = F.interpolate(pred12, size=shape, mode='bilinear')
        pred22 = F.interpolate(pred22, size=shape, mode='bilinear')
        out22h = F.interpolate(out22h, size=shape, mode='bilinear')
        out32h = F.interpolate(out32h, size=shape, mode='bilinear')
        out42h = F.interpolate(out42h, size=shape, mode='bilinear')
        out52h = F.interpolate(out52h, size=shape, mode='bilinear')
        ...
        return pred1+pred12, pred2+pred22, out2h+out22h, out3h+out32h, out4h+out42h, out5h+out52h
